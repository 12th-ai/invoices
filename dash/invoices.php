  ...
  <main>
    ...
    <div class="main">
      <?php 
        ...
        if(isset($_GET['del'])) {
          $delId = $_GET['del'];
          $del = mysqli_query($conn, "DELETE FROM invoice WHERE invoice_number = '$delId'");
          if($del) {
            echo '<script>
              Swal.fire({
        ...
        if(isset($_POST['searchq'])){
          ...
          $urlParams = '?dateFilter&s='.$start.'&e='.$end;
        } else {
      ...
      <div class="main-data">
        <div class="data-table">
          <h1 class="title">
            All Invoices (<?php echo mysqli_num_rows($get); ?>)
            <?php 
              if($_SESSION['userLogLvl'] == 1 || $_SESSION['userLogLvl'] == 0){
            ?>
            <a href="./invoices_add.php">
              Add Invoice
          ...
          <div class="filters">
            <div class="each-filter">
              <form action="" method="POST">
                <input type="search" name="search" value="<?php if(isset($_POST['searchq'])){echo $key; }?>" required placeholder="Search (Owner, Amount, etc)" id="">
                <button name="searchq">Search</button>
              </form>
            </div>
            <?php 
              if(isset($_POST['search']) || isset($_POST['dateFilter'])){
            ?>
            <a href="./invoices.php" class="clear">Clear Filters</a>
            <?php } ?>
            <div class="each-filter">
              <form action="" method="POST">
                <input type="date" name="start" value="<?php if(isset($_POST['dateFilter'])){echo $start; }?>" required id="">
                <input type="date" name="end" value="<?php if(isset($_POST['dateFilter'])){echo $end; }?>" required id="">
                <button name="dateFilter">Filter Date</button>
              </form>
            </div>
          </div>
          <?php 
          ...
          <table>
            ...
            ?>
            <tr>
              <td><?php echo $fetch['invoice_number'] ?></td>
              <td><?php echo number_format($fetch['amount'], 0, ',',',')." ".$fetch['currency']; ?></td>
              ...
              ?>
              <td>
                <a href="./invoices_update.php?upId=<?php echo $fetch['invoice_number'];?>">Update</a>
                <button onclick="deleteRecord(<?php echo $fetch['invoice_number']; ?>)">Delete</button>
              </td>
              <?php } ?>
            </tr>
            <?php } ?>
          </table>
          <a href="<?php echo 'invoices_export.php'.$urlParams?>" class="reportBtn">
            <button>Export Report</button>
          </a>
          <?php } else { ?>
            <p class="center">No Invoices Found</p>
          <?php } ?>
        </div>
      </div>
    </div>
    ...
    function deleteRecord(deleteId) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location.href = './users.php?del='+deleteId;
        }
      })
    }
  </script>
